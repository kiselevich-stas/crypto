<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="fixed d-none w-100 h-100 opacity-80 bg-purple-800 inset-0 z-50 flex items-center justify-center">
    ...
    <div class="container">
      <section>
        <div class="">
          ...
          <input type="text" name="wallet" id="wallet" v-model="ticker" @keydown.enter="add"
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div class="bg-white overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer"
            v-for=" t in tickers" v-bind:key="t.name" @click="sel = t" :class="sel == t ? 'border-4' : ''">
      ...
      <section v-if="sel" class="relative">
        ...
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div class="bg-purple-800 border w-10 h-24" v-for="(bar,idx) in normilizeGraph()" :key="idx" :style="{ height: `${bar}%` }"></div>
        </div>
...
export default {
  ...
  methods: {
    add() {
      ...
      setInterval(async () => {
        const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=228379e2842cbd6cdb76cd0769079ab0a147b6366ed7027ff908f6e9c1c86775`);
    ...
    normilizeGraph(){
      ...
      console.log(this.graph.map(price => ((price - minValue) * 100) / (maxValue - minValue)));
